...
//
// var billTotal = parseFloat(prompt("What was the total of your bill"));
//
// alert("Your lucky number was " + luckyNumber);
// alert("Price Before Discount: $" + billTotal.toFixed(2));
// alert("Price after discount: $" + calculateTotal(luckyNumber, billTotal).toFixed(2));
//
// // BONUSES
//
...
function isValidPassword(input) {
    ...
    var specialPattern = new RegExp("[^\w\d\s]");

    var hasLower = lowerPattern.test(input);
    var hasUpper = upperPattern.test(input);
    var hasNum = numPattern.test(input);
    var hasSpecial = specialPattern.test(input);

    if( input.length > 5 && hasLower && hasUpper && hasNum && !hasSpecial)
        return true;

    return false;
}

console.log(isValidPassword("This is a shitty password ; 69"));
